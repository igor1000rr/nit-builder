 ...
 *   - Сохраняем термины с дефисом (b2b, кофе-на-вынос) и цифры (7.0, 450, 4)
 *   - Одиночные цифры (балл "4", класс "9") сохраняются — они часто несут смысл
...
const RU_STOPWORDS = new Set([
  ...
  "или", "а", "не", "нет", "бы", "был", "была", "были", "будет",
  "все", "всех", "всего", "всем", "что", "чтоб", "чтобы", "же", "ж", "ли",
  "ведь", "хотя", "их", "им", "он", "она", "они", "мы", "вы", "я",
]);
...
function stemRussian(token: string): string {
  ...
  const suffixes = [
    "ями", "ях", "ия", "ея", "ью", "ии", "ев", "ов", "ым", "им",
...
export function tokenize(text: string): string[] {
  ...
  for (const tok of raw) {
    ...
    // Пропускаем пустые. Одиночные символы пропускаем кроме чисто цифровых
    // ("4 балла", "9 класс" — цифра несёт смысл).
    if (!cleaned) continue;
    if (cleaned.length < 2 && !/^\d+$/.test(cleaned)) continue;
    if (RU_STOPWORDS.has(cleaned) || EN_STOPWORDS.has(cleaned)) continue;
    // Только цифры (и/или точка) без букв — важны (7.0, 2024, 450, 4)
    if (/^[\d.]+$/.test(cleaned)) {
      out.push(cleaned);
      continue;
    }
